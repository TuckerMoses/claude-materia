...
# is overwritten on every park (single-user, single-machine workflow); within a
# single-invocation scope, a stale sentinel from a prior park is harmless
# because this park re-derives and rewrites it before its own extension path
# is reachable. (Concurrent parks can race on the sentinel; see Known
# limitations.) The dotfile prefix signals skill-managed metadata, distinct
# from user-editable config.yaml in the same directory; `ls` will not show it
# without `-a`. Accumulation is bounded to one file (overwritten in place), so
# no cleanup is needed.
# The mkdir -p is defensive: init creates this directory at config-write
# time, but a park invocation against a freshly-cleared data dir (or one
# where the user removed the dir between init and park) would otherwise fail
# the sentinel write. Cheap and idempotent.
...
printf '%s\n' "$active_transcript" > "$HOME/.claude/plugins/data/claude-materia-claude-materia/park-session/.active-transcript" || {
  echo "park: failed to write sentinel file at $HOME/.claude/plugins/data/claude-materia-claude-materia/park-session/.active-transcript (disk full, permissions). Aborting park before drafting." >&2
  exit 1
}
...
**Known fragility:** the probe-and-grep fallback depends on Claude Code logging bash tool invocations verbatim into the per-session JSONL transcript. This is undocumented internal harness behavior and may break across Claude Code versions; the fail-loud guards above are the defense. See the corresponding bullet under Known limitations for the formal statement.

### 3. Process the transcript tail and draft entry fields

Step 2's consolidated block emitted the chosen transcript's last 40 lines on stdout. Step 3 consumes step 2's output — the only bash here is the conditional `tail -120` extension on the unhappy path (3(d) below). All other work is prose-only: process the captured tail to synthesize the entry fields. (Folding the tail-read into step 2 excises the cross-Bash-call handoff that previously discarded the disambiguation result on the probe-and-grep path.)

Process the captured tail in this order: (a) take the 40 lines emitted by step 2d as the working set; (b) filter out tool-result events larger than 4 KB (typically large file reads or command output) — they bloat the read budget without adding signal; (c) within the filtered set, count **substantive events** as the union of (i) `tool_use` events and (ii) assistant events whose text content exceeds 200 chars — each JSONL line counts at most once; (d) if the count is below 3, **re-tail only — do not re-run the full disambiguation cascade**. Read the sentinel file written at the end of step 2d and tail it directly: `tail -120 "$(< ~/.claude/plugins/data/claude-materia-claude-materia/park-session/.active-transcript)"` (the bash `$(< file)` builtin form avoids a `cat` subprocess). Re-filter the 120-line working set, stopping after at most 120 lines total. **If the post-extension count is still below 3, apply the low-confidence prefix per Edge cases.** The prefix fires only after the extension fails to recover; a 120-line tail that finds ≥3 substantive events is treated as a confident draft and no prefix is applied.
...
- **Concurrent park sentinel race (and sentinel-missing).** The `.active-transcript` sentinel written by `park` step 2d (see consolidated block) is shared across concurrent park invocations against the same skill data dir; it is last-writer-wins. If park-A writes the sentinel at step 2d and park-B overwrites it before park-A reaches step 3(d)'s `tail -120`, park-A reads park-B's transcript path and synthesizes its draft fields from the wrong session — a wrong-session data leak into a draft entry, distinct in kind from the destination-file clobbering race above. **Detection** is best-effort and not automatic: the agent drafting the entry may notice that the tail content does not match the active session's recent activity (different files, different topic) and surface the mismatch to the user during the step-6 review pane. There is no programmatic sentinel-ownership check; if the sessions are similar enough, the mismatch can slip through. **Mitigation:** serialize parks across concurrent sessions (e.g., one park at a time per machine) when the unhappy-path 120-line extension might trigger. For draft entries that hit the happy path (≥3 substantive events in the 40-line tail emitted by step 2d itself), the sentinel is never read and the race is moot. Separately: if the sentinel is missing or empty when step 3(d) reads it (e.g., an external process such as a cleanup script or the user's `rm` removed it between step 2d's write and step 3(d)'s read), the `tail -120 "$(< ... )"` resolves to an empty path and fails opaquely. The agent should treat this as a degraded-extension condition, fall back to processing only the 40-line tail captured at step 2d's stdout, and apply the low-confidence prefix per Edge cases — do not attempt to re-derive the transcript path.
