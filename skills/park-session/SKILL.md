...
- **Concurrent park during audit:** audit reads the fenced block once at start, then makes per-entry decisions. If a `park` invocation appends a new entry between audit's read and audit's writes, audit may overwrite the new entry on its next per-entry write. The skill targets a single-user, single-machine workflow where this race is unlikely; users running concurrent park and audit (e.g., across multiple tmux sessions) should sequence them manually.
