...
| **vault** | Command | Portable knowledge-vault skill. `create` scaffolds a born-correct vault (flat `notes/` + `journal/`, parseable `_machine/labels.yml`, `INSTRUCTION.md` with local path filled, `.obsidian/` config) with a four-phase workflow (Analyze → Structure-lock → Scaffold → File). `add-source` ingests an external corpus into an existing vault. `discuss` routes vault strategy questions to the right advisor (kind-bootstrapper or inline). Binds to `~/.claude/vault.local.md`. |
